...
double ComplexRedundantSystem::doTest()
{
	double workTime = 0;
	double p = 1;
	...
	mainElementSet.reset(workTime, testTime_);
	spareElementSet.reset();
	repairUnitSet.reset();
	std::queue<int> empty;
	std::swap(repairQueue, empty);
	
	double lastPoint = workTime;
	...
	while ((workTime < testTime_) && !crash)
	{
		...
		if (mainElementCrushTime < testTime_)
		{
			...
			{				
				p *= mainElementSet.getDeltaProbability(testTime_ - lastPoint);
				...
				else
				{
					...
					mainElementSet.reset(closestMainElement, workTime, testTime_);
